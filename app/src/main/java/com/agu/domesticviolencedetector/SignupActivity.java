...
public class SignupActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener, AdapterView.OnItemClickListener {
    ...
    public void signup() {
        ...
        if(_genderSpinner.getSelectedItem().toString().equals("Male"))
            gender=1;
        final Integer age = Integer.parseInt(_ageText.getText().toString());
        ...
        mFirebaseAuth.createUserWithEmailAndPassword(email,password).addOnCompleteListener(SignupActivity.this, new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(!task.isSuccessful()){
                    ...
                    int dangerLevel = predictDanger(name,email,age, finalGender);
                    ...
                    User user = new User(name,email,age, finalGender, dangerLevel);
                    ...
                    FirebaseDatabase.getInstance().getReference("users")
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if(task.isSuccessful()){
                                Toast.makeText(SignupActivity.this, "Database Successfull", Toast.LENGTH_SHORT).show();
                            }else {
                                Toast.makeText(SignupActivity.this, "Register Database Error", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                    //addUser(user);
...
//                }, 3000);
    ...
    public int predictDanger(String name,String email, int age, int finalGender) {
        final int[] result = {0};
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    jsonParam.put("age-of-suspect", 2);
                    jsonParam.put("education-level-of-suspect", 2);
                    jsonParam.put("harmful-use-of-alcohol", 2);
                    jsonParam.put("threatens-with-violence", 1);
                    jsonParam.put("welfare-level", 1);
                    jsonParam.put("blame", 0);
                    jsonParam.put("violence-history", 0);
    ...
    public void sendPost(final User user) {
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    ...
                    int x = (int) ((Math.random()*((12345678-12000000)+1))+12000000);

                    jsonParam.put("userId", x);
                    jsonParam.put("name", user.getName());
                    jsonParam.put("gender", user.getGender());
                    jsonParam.put("age", user.getAge());
                    jsonParam.put("location", "Burdur");
